...
export class SupabaseModule extends Module {
    ...
    async isComplete(): Promise<boolean> {
        try {
            ...
            const statusResult = await pnpx("supabase", ["status"], {
                timeout: 10000,
            });
            ...
            const migrationResult = await pnpx(
                "supabase",
                ["migration", "list"],
                { timeout: 5000 }
            );
     ...
     */
    private async startWithProgress(): Promise<{
        exitCode: number;
        output: string;
        error?: string;
    }> {
        ...
        return new Promise(resolve => {
            ...
            const updateProgress = () => {
                ...
                const totalBars = 30;
                const percentage =
                    layers.size > 0 ? completedLayers.size / layers.size : 0;
            ...
            const parseDockerOutput = (data: string) => {
                ...
                for (const line of lines) {
                    ...
                    const layerMatch = line.match(
                        /([a-f0-9]{12}):\s*(Pulling|Waiting|Downloading|Verifying|Download complete|Pull complete|Already exists)/i
                    );
    ...
    private async ensureMigrations(): Promise<void> {
        ...
        const migrationList = await pnpx("supabase", ["migration", "list"], {
            timeout: 10000,
        });

        // Check if there are pending migrations
        if (
            !migrationList.output.includes("pending") &&
    ...
    private parseSupabaseStatus(output: string): SupabaseCredentials {
        ...
        for (const line of lines) {
            ...
            if (
                trimmed.includes("Publishable key") ||
