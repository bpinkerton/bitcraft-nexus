...
async function setupSpacetime(options = {}) {
    ...
    try {
        ...
        const schemaUrl = 'https://bitcraft-early-access.spacetimedb.com/v1/database/bitcraft-global/schema?version=9';
        const schemaData = execSync(`curl -s "${schemaUrl}"`, { encoding: 'utf-8' });
        const schemaObj = { V9: JSON.parse(schemaData) };
        fs.writeFileSync(path.join(SPACETIME_DIR, 'schema.json'), JSON.stringify(schemaObj, null, 2));
...
async function setupAuthentication() {
    ...
    if (fs.existsSync(ENV_FILE)) {
        ...
        if (envContent.includes('SPACETIME_AUTH_TOKEN=')) {
            const skip = await question('Auth token already exists in .env.local. Skip auth setup? (Y/n): ');
    ...
    try {
        ...
        if (!email || !email.includes('@')) {
            ...
            console.log('   You can run this later with: pnpm run setup:spacetime --force\n');
            rl.close();
            return;
        }
        ...
        const accessCode = await question('🔑 Enter the access code from your email: ');
        ...
        try {
            ...
            authToken = authData.token || authData.authToken || authResponse.trim();
        } catch {
            authToken = authResponse.trim();
        }

        if (!authToken || authToken.length < 10) {
            console.log('❌ Failed to get auth token. Invalid response from server.');
            console.log('   You can manually add SPACETIME_AUTH_TOKEN to .env.local\n');
            rl.close();
            return;
        }
